...
CREATE POLICY "Public read active gallery" ON gallery_photos FOR SELECT USING (active = true);
CREATE POLICY "Admin full access gallery" ON gallery_photos FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Public read availability" ON availability_blocks FOR SELECT USING (true);
CREATE POLICY "Admin full access availability" ON availability_blocks FOR ALL USING (auth.role() = 'authenticated');
...
CREATE POLICY "Public can upload booking images" ON booking_images FOR INSERT WITH CHECK (true);
CREATE POLICY "Public can read booking images" ON booking_images FOR SELECT USING (true);
CREATE POLICY "Admin full access booking_images" ON booking_images FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Public read settings" ON admin_settings FOR SELECT USING (true);
CREATE POLICY "Admin full access settings" ON admin_settings FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Public read active policies" ON policies FOR SELECT USING (active = true);
CREATE POLICY "Admin full access policies" ON policies FOR ALL USING (auth.role() = 'authenticated');
CREATE POLICY "Public can subscribe" ON push_subscriptions FOR INSERT WITH CHECK (true);
CREATE POLICY "Admin full access push_subscriptions" ON push_subscriptions FOR ALL USING (auth.role() = 'authenticated');
