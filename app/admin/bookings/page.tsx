  ...
  return (
    <div className="px-4 py-6">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-5 no-scrollbar">
        {FILTERS.map((f) => (
          <Link key={f} href={f === 'all' ? '/admin/bookings' : `/admin/bookings?status=${f}`}
            className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium capitalize transition-colors ${filter === f ? 'bg-[#ffabdd] text-[#1a1a1a]' : 'bg-[#222] border border-[#3a3a3a] text-white/60 hover:border-[#ffabdd]/40'}`}>
            {f}
          </Link>
        ))}
      </div>
